...
const NavItem = ({ isExpanded, isActive, organization, onExpand }: NavItemsProps) => {
  ...
  return (
     <AccordionItem value={organization.id} className="border-none">
       ...
       <AccordionContent className="pt-1 text-nutral-700">
         {routes.map((route) => (
            <Button key={route.href} onClick={() => onClick(route.href)} className={cn("w-full font-normal justify-start pl-10 mb-1", pathname === route.href && "bg-sky-500/10 text-sky-700")} variant="ghost">
                {route.icon}
                {route.label}
            </Button>
         ))}
